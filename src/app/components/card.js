"use client";

import { useContext, useState } from "react";
import AppContext from "../context/appContext";
import Form from "./form";

function Card({ task, opacity, setOpacity }) {
  const { dispatch, setTitle, setPriority, setDescription, setDueDate } =
    useContext(AppContext);
  const [editTaskId, setEditTaskId] = useState(null);
  const handleTaskEdit = (id, title, description, dueDate, priority) => {
    setTitle(title)
    setPriority(priority)
    setDescription(description)
    setDueDate(dueDate)
    setEditTaskId(id);
    setOpacity("1");
  };
  return (
    <>
      <Form
        opacity={opacity}
        setOpacity={setOpacity}
        editTaskId={editTaskId}
        setEditTaskId={setEditTaskId}
      />
      {task ? (
        <div className="w-auto h-auto w-full bg-white border border-gray-200 rounded-lg  dark:bg-gray-800 dark:border-gray-700">
          {/* TASK DELETE< STATUS< EDIT BUTTON */}
          <div className="flex justify-between px-4 dropdown pt-4 z-0">
            {task.createdAt && (
              <>
                <span className="text-xs inline-block text-gray-500 text-sm dark:text-gray-300
                z-0">
                  Date: {task.createdAt}
                </span>
                <button className="inline-block dropbtn text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-xs">
                  <span className="sr-only">Open dropdown</span>
                  <svg
                    className="w-5 h-5"
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="currentColor"
                    viewBox="0 0 16 3"
                  >
                    <path d="M2 0a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm6.041 0a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM14 0a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Z" />
                  </svg>
                </button>
                <div
                  id="dropdown"
                  className="z-10 dropdown-content hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700"
                >
                  <ul className="py-2" aria-labelledby="dropdownButton">
                    <li>
                      <button
                        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                        onClick={() => handleTaskEdit(task.id, task.title, task.description, task.dueDate, task.priority)}
                      >
                        Edit
                      </button>
                    </li>
                    <li>
                      <button
                        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                        onClick={() =>
                          dispatch({ type: "TOGGLE_STATUS", payload: task.id })
                        }
                      >
                        {task.status == false ? (
                          <span>Pending</span>
                        ) : (
                          <span>Success</span>
                        )}
                      </button>
                    </li>
                    <li>
                      <button
                        className="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                        onClick={() =>
                          dispatch({ type: "DELETE_TASK", payload: task.id })
                        }
                      >
                        Delete
                      </button>
                    </li>
                  </ul>
                </div>
              </>
            )}
          </div>
          <div className="flex flex-col px-4 pb-10">
            <p className="mb-1 font-bold text-sm text-gray-900 dark:text-white">
              {task.title}
            </p>
            <p className="text-sm text-gray-500 text-sm dark:text-gray-400">
              {task.description}
            </p>
            <div className="flex justify-between pt-4">
              {task.dueDate ? (
                <span className="text-sm text-gray-500 text-xs align-middle dark:text-gray-300">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    width={"24px"}
                    className="inline me-2 align-middle"
                  >
                    <path d="M381.2 64.1c-1.3-.1-2.6-.1-3.9-.1h-.2c-16.2 0-32 5.4-44.6 15.1-1.6 1.3-2.6 3.2-2.7 5.2-.1 2 .8 4 2.3 5.4l89.8 80.5c1.3 1.1 2.9 1.8 4.6 1.8h.4c1.9-.1 3.6-1 4.8-2.4C440.9 159 448 150.8 448 133c.1-36.4-29.1-66.8-66.8-68.9zM64 133c0 17.8 7.1 26 16.3 36.6 1.2 1.4 2.9 2.3 4.8 2.4h.4c1.7 0 3.3-.6 4.6-1.8L180 89.7c1.5-1.4 2.4-3.3 2.3-5.4-.1-2-1-3.9-2.7-5.2C167 69.4 151.2 64 135 64h-.2c-1.3 0-2.6 0-3.9.1-37.7 2.1-67 32.5-66.9 68.9z" />
                    <g>
                      <path d="M390 386c26.2-30.7 42-70.5 42-114 0-97.2-78.8-176-176-176S80 174.8 80 272c0 43.5 15.8 83.3 42 114l-34.7 35.5c-6.2 6.3-6 15.5.3 21.6 3.1 3 7.4 4.8 11.4 4.8 4.2 0 8.1-1.9 11.2-5.1l34.6-34.5c30.3 24.7 69 39.6 111.2 39.6s80.9-14.8 111.2-39.6l33.6 34.5c3.1 3.2 7.3 5.1 11.5 5.1 4 0 8.1-1.8 11.2-4.8 6.3-6.2 7.5-15.3 1.3-21.6L390 386zM270 274c0 7.7-6.3 14-14 14h-82c-7.7 0-14-6.3-14-14s6.3-14 14-14h68V158c0-7.7 6.3-14 14-14s14 6.3 14 14v116z" />
                    </g>
                  </svg>
                  {task.dueDate}
                </span>
              ) : null}
              <span
                className={`text-sm px-4 py-1 rounded-sm text-sm dark:text-gray-300 ${
                  task.priority == "Medium"
                    ? "text-yellow-800 bg-yellow-100 "
                    : task.priority == "High"
                    ? "text-red-800 bg-red-100"
                    : task.priority == "Low"
                    ? "text-green-800 bg-green-100"
                    : null
                }`}
              >
                {task.priority}
              </span>
            </div>
            <span className="text-sm text-right px-1 mt-2 text-gray-500 text-xs align-middle dark:text-gray-300">
              {!task.status ? <i>pending</i> : <i>success</i>}
            </span>
          </div>
        </div>
      ) : null}
    </>
  );
}

export default Card;
